...
if( isset($_POST["user"]) &&
    isset($_POST["pass"]) ) {
        ...
        if( $user->login() ) {
            ////caso o login retorne "true", inicializa-se o gerenciamento de sessoes
            session_start();
            //é apresentado um alert java script
            echo "<script> alert('AUTENTICADO! ✅') </script>";
            //É criada uma sessão com os dados do objeto retornado
            $_SESSION["user"] = $user->getObject();
            //E então somos redirecionados para pagina de perfil
